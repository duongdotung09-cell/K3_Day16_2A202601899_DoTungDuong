...
class CitationChecker(Middleware):
    ...
    def _matches_a_line(self, text: str, body: str) -> bool:
        """`text` có khớp nguyên văn (là substring của) một DÒNG của `body`?"""
    ...
    def after_agent(self, ctx, report):
        ...
        for claim in claims:
            ...
            if current is not None and self._matches_a_line(text, current.body):
                continue  # trích dẫn đã đúng, giữ nguyên

            for doc in ctx.corpus.docs:
                if doc.body in ctx.observed_text and self._matches_a_line(text, doc.body):
                    claim["doc_id"] = doc.doc_id  # gắn lại, KHÔNG sửa text
                    break
